    ...
    OnlineImage.prototype = {
        ...
        initEvents: function () {
            ...
            domUtils.on(this.container, 'click', function (e) {
                ...
                if (!domUtils.hasClass(li, 'folder')) {
                    ...
                    _this.listIndex = 0;
                    _this.listEnd = false;
                    _this.listPath =li.getAttribute('_src');
                    _this.getImageData();
                }
            });
        },
        ...
        initData: function () {
            ...
            /* 拉取数据需要使用的值 */
            this.state = 0;
            this.listPath = editor.getOpt('imageManagerListPath');
            this.listIndex = 0;
            this.listEnd = false;
        ...
        getImageData: function () {
            ...
            if(!_this.listEnd && !this.isLoadingData) {
                ...
                var url = editor.getActionUrl(editor.getOpt('imageManagerActionName')),//editor.getOpt('serverUrl'),
        ...
        pushData: function (list) {
            var i, item, img, del, icon, _this = this,
                urlPrefix = editor.getOpt('imageManagerUrlPrefix');
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    list[i].url = decodeURIComponent(list[i].url);
                    ...
                    img.width = 113;
                    var _title = list[i].url.slice(list[i].url.lastIndexOf('/') + 1);
                    item.setAttribute('title', _title);
                    if (/\.(jpg|jpeg|bmp|gif|png)/i.test(list[i].url)) {
                        ...
                        del.setAttribute('alt',list[i].url);
                        del.setAttribute('href','javascript:;');
                        domUtils.on(del, 'click', (function(){
                            return function(){
                                var _del=this;
                                var delurl = editor.getActionUrl(editor.getOpt('imageDelActionName')),isJsonp = utils.isCrossDomainUrl(delurl);
                                var listurl=_del.getAttribute('alt');
                        ...
                        img.setAttribute('src', urlPrefix + '/static/ueditor/themes/default/images/folder.png' );
                        img.setAttribute('class', 'icon-folder');
                        item.setAttribute('_src', urlPrefix + list[i].url);
                        domUtils.addClass(item, 'folder');
                        domUtils.addClass(icon, 'icon');
                        icon.innerHTML=_title;
                        item.appendChild(img);
                        item.appendChild(icon);
                    }
